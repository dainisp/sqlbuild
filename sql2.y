...
extern void enable_sel(int sel);
extern void save_table(char * table,char * alias);
extern void save_sel_cols(char * table,char * column);
...
/*
select_statement:
                SELECT opt_all_distinct  { enable_sel(1); }   selection { enable_sel(0); }
		opt_into_statement
		table_exp
	;
...
query_spec:
                SELECT  opt_all_distinct   { enable_sel(1); }  selection  { enable_sel(0); } table_exp
	;

selection:
                scalar_exp_opt_alias_commalist
	|	'*'
 ;
...
table_ref:
                table { save_table($1,0) ; }
        |	table range_variable {save_table($1,$2) ;  }
	;

where_clause:
                WHERE  { enable_where(1); }  search_condition  { enable_where(0); }
	;
...
scalar_exp_opt_alias_commalist:
        scalar_exp_opt_alias
        |  scalar_exp_opt_alias_commalist ',' scalar_exp_opt_alias
        ;
